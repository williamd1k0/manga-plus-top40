...
if (ARGS.includes("-a")) {
  ...
  nowDate = [
    archiveID.substr(0, 4),
    archiveID.substr(4, 2),
    archiveID.substr(6, 2),
  ].join("-");
}
...
(async () => {
  ...
  if (URL.endsWith("/manga_list")) {
    // deprecated since 2023.03.28
  ...
  await page.waitForSelector("div[class^=HotTitle-module_container]", {
    timeout: 5000,
  });
  ...
  for (let entry of entries) {
    const title = await entry.$eval(
      "p[class^=HotTitle-module_title]",
      (e) => e.innerText,
    );
